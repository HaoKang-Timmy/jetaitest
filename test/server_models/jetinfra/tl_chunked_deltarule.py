...
def tl_chunk_cumsum(
    ...
    def kernel(
        ...
        with T.Kernel(T.ceildiv(Token, Block_T), B * H, threads=threads) as (bt, bbh):
            ...
            InputG_fragment = T.view(InputG_fragment, )
            T.copy(InputG_fragment, InputG_shared)
            T.copy(InputG_shared, OutputG[bb, bt * Block_T:(bt + 1) * Block_T, bh])
    return kernel
if __name__ == "__main__":
    Input = torch.randn(1, 32768, 32, dtype=torch.bfloat16).cuda()
    kernel = tl_chunk_cumsum(1, 32768, 32)
    Output = torch.empty(1, 32768, 32, dtype=torch.float32).cuda()
    kernel(Input, Output)
    print(Output)
