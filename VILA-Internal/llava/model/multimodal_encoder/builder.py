...
def build_vision_tower(model_name_or_path: str, config: PretrainedConfig) -> PreTrainedModel:
    ...
    elif "siglip" in vision_tower_name and "siglip2" not in vision_tower_name:
        # NOTE(ligeng): only for siglip models
        logger.info(f"Using siglip vision tower: {model_name_or_path}")
        if use_dynamic_s2:
    ...
    elif "siglip2" in vision_tower_name:
        logger.info(f"Using siglip2 vision tower: {model_name_or_path}")
        if use_dynamic_s2:
            raise NotImplementedError("Dynamic S2 is not supported for siglip2 models")
        elif use_s2:
            raise NotImplementedError("S2 is not supported for siglip2 models")
        else:
            vision_tower = Siglip2VisionTower(model_name_or_path, config)
    else:
