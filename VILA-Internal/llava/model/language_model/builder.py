...
def build_llm_and_tokenizer(
    ...
    use_local_model=True,  # 新增参数：优先使用本地模型
    *args,
    **kwargs,
) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:
    # print(model_name_or_path)
    # NOTE(ligeng): load remote code by default. This may execute malicious code.
    # 配置加载仍需 trust_remote_code=True（配置文件可能包含自定义代码）
    # 但模型加载时，如果 use_local_model=True，会使用本地类
    llm_cfg = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=True)
    # 模型加载时的 trust_remote_code 取决于是否使用本地模型
    kwargs["trust_remote_code"] = not use_local_model
    ...
    if quantization_restore_from_checkpoint:
        ...
        if use_local_model and architectures and "JetNemotron" in architectures[0]:
            from .jet_nemotron.modeling_jet_nemotron import JetNemotronForCausalLM
            from .jet_nemotron.configuration_jet_nemotron import JetNemotronConfig
            
            llm_cfg = JetNemotronConfig(**llm_cfg.to_dict())
            llm_cfg._attn_implementation = attn_implementation
            
            # 使用本地模型时，移除 trust_remote_code 参数
            local_kwargs = {k: v for k, v in kwargs.items() if k != "trust_remote_code"}
            llm = JetNemotronForCausalLM.from_pretrained(
                fp8_model_name_or_path, config=llm_cfg, torch_dtype=eval(config.model_dtype), *args, **local_kwargs
            )
        else:
        ...
        if use_local_model and architectures and "JetNemotron" in architectures[0]:
            ...
            local_kwargs = {k: v for k, v in kwargs.items() if k != "trust_remote_code"}
            llm = JetNemotronForCausalLM.from_pretrained(
                model_name_or_path, config=llm_cfg, torch_dtype=eval(config.model_dtype), *args, **local_kwargs
            )
        else:
            llm = AutoModelForCausalLM.from_pretrained(
    ...
    if getattr(config, "chat_template", None) is not None:
        ...
        logger.info(f"Using chat template: {config.chat_template} {fpath}")
